...
class UserRegisterSerializer(serializers.ModelSerializer):
	...
	confirm_password = serializers.CharField(min_length=6, max_length=16, write_only=True)
	token = serializers.CharField(label='token', read_only=True)

	class Meta:
		model = User
		fields = ['id', 'username', 'password', 'mobile', 'sms_code', 'allow', 'confirm_password', 'token']
		extra_kwargs = {
			'username': {
				'min_length': 5,
				'max_length': 20,
